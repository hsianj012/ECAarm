...
%% Establish waypoints
alpha = deg2rad([40,30,20,10]); % shoulder
gamma = deg2rad([40,30,20,10]); % Slew
beta = deg2rad([40,30,20,10]);  % Elbow

% alpha = deg2rad([10,20,30,40]); % shoulder
% gamma = deg2rad([10,20,30,40]); % Slew
% beta = deg2rad([10,20,30,40]);  % Elbow
% % figure()
...
demand_type = [2 1 1 0 0];
% set demand values for first waypoint
demand = [  100, 100, 100, 0, 0];
